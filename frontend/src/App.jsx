        ...
        <div className="result">
          {result.filters ? (
            <>
              ...
              <pre className="filters">{JSON.stringify(result.filters, null, 2)}</pre>
            </>
          ) : (
              ...
              {result.results.map((car) => (
                <div key={car.id} className="car-card" onClick={() => handleCarClick(car.id)}>
                  ...
                  <div className="car-details">
                    {car.engine} · {car.year} · {car.price.toLocaleString('ru-RU')} ₽
                  </div>
          ...
          <div className="metrics">
            Метод: {result.metrics.parsing_method} · Время: {result.metrics.latency_ms}ms · Стоимость: ${result.metrics.cost_usd}
          </div>
        </div>
      )}
        ...
        <div className="modal" onClick={() => setSelectedCar(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">{selectedCar.mark_name} {selectedCar.folder_name}</span>
              <button className="modal-close" onClick={() => setSelectedCar(null)}>✕</button>
            </div>
            <div className="detail-row"><span>Кузов</span><span>{selectedCar.body_type}</span></div>
            <div className="detail-row"><span>Двигатель</span><span>{selectedCar.engine_volume}L {selectedCar.engine_type}</span></div>
            <div className="detail-row"><span>Мощность</span><span>{selectedCar.hp} л.с.</span></div>
            <div className="detail-row"><span>КПП</span><span>{selectedCar.transmission}</span></div>
            <div className="detail-row"><span>Привод</span><span>{selectedCar.drive_type}</span></div>
            <div className="detail-row"><span>Год</span><span>{selectedCar.year}</span></div>
            <div className="detail-row"><span>Цена</span><span>{selectedCar.price.toLocaleString('ru-RU')} ₽</span></div>
          </div>
        </div>
      )}
    </>
  ...
  return (
    <div className="dashboard">
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-value">{((analytics?.today?.parsing_accuracy || 0) * 100).toFixed(1)}%</div>
          <div className="stat-label">Точность парсинга</div>
        </div>
        <div className="stat-card">
          <div className="stat-value">${(analytics?.today?.llm_cost_usd || 0).toFixed(4)}</div>
          <div className="stat-label">Стоимость LLM</div>
        </div>
        <div className="stat-card">
          <div className="stat-value">{analytics?.catalog?.total_records || 0}</div>
        ...
        <div className="top-brands">
          ...
          {analytics.top_brands.map((brand, i) => (
            <div key={brand.name} className="brand-row">
              ...
              <span>{brand.count} ({(brand.share * 100).toFixed(0)}%)</span>
            </div>
          ))}
        </div>
      )}

      <button className="refresh-btn" onClick={fetchAnalytics}>Обновить</button>
    </div>
  )
}
  ...
  const handleAddSynonym = async () => {
    ...
    try {
      await fetch(`${API_URL}/api/v1/admin/prompts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ synonyms: { [newSynonym.key]: newSynonym.value } })
      })
    ...
    <div className="prompt-editor">
      ...
      <div className="synonyms-section">
        ...
        <div className="synonyms-list">
          {config?.synonyms && Object.entries(config.synonyms).map(([key, value]) => (
        ...
        <div className="add-synonym">
          ...
          />
          <button onClick={handleAddSynonym} disabled={saving}>+</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="container">
      ...
      <div className="nav">
        ...
        <button className={page === 'prompts' ? 'active' : ''} onClick={() => setPage('prompts')}>
          Промпты
        </button>
      </div>
